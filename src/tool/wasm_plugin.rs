...
impl Tool for WasmPluginTool {
    ...
    async fn execute(&self, args: HashMap<String, Value>) -> ToolResult {
        ...
        let result: Result<(Vec<u8>, Vec<u8>), anyhow::Error> =
            tokio::task::spawn_blocking(move || -> Result<(Vec<u8>, Vec<u8>), anyhow::Error> {
                ...
                match start.call(&mut store, ()) {
                    ...
                    Err(e) => {
                        ...
                        if msg.contains("fuel") || msg.contains("exhausted") {
                            Err(anyhow::anyhow!(
                                "WASM plugin exhausted fuel limit of {}",
                                fuel
                            ))
                        } else {
                            Err(e)
        ...
        match result {
            ...
            Err(e) => ToolResult::error(format!("WASM Error: {}", e)),
        }
    }
}
...
pub async fn load_wasm_plugins(dir: &Path) -> Vec<(WasmPluginConfig, PathBuf)> {
    ...
    while let Ok(Some(entry)) = entries.next_entry().await {
        ...
        match tokio::fs::read_to_string(&manifest_path).await {
            ...
            Err(e) => {
                tracing::warn!(
                    "Failed to read WASM plugin file {}: {}",
                    manifest_path.display(),
                    e
                );
            }
...
pub async fn register_wasm_plugins(
    registry: &ToolRegistry,
    plugins: Vec<(WasmPluginConfig, PathBuf)>,
) {
...
mod tests {
    ...
    async fn test_wasm_plugin_execution() {
        ...
        assert!(
            result.for_llm.contains("Hello, Antigravity!"),
            "Actual output: {}",
            result.for_llm
        );
    }
    ...
    async fn test_wasm_plugin_fuel_exhaustion() {
        ...
        assert!(
            result.is_error,
            "Expected fuel exhaustion error, got success: {}",
            result.for_llm
        );
        assert!(
            result.for_llm.contains("exhausted fuel limit")
                || result.for_llm.contains("WASM Error"),
            "Actual error: {}",
            result.for_llm
        );
    }
